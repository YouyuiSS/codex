...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    ...
    // codex-tea fork: `ApplyPatchHandler` 在 fork 内被扩成两 spec 形态
    // (Freeform/Function)，由 `model_info.apply_patch_tool_type` 选择。chat-wire
    // provider (DeepSeek/GLM/Qwen 等) catalog 必须显式声明 Function，否则
    // 这里不会注册 apply_patch tool（与上游"无声不注册"行为一致）。
    if environment_mode.has_environment()
